...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while(!toExplore.isEmpty()) {
			...
			if(!visited.contains(curr)) {
				...
				HashMap<MapNode, Double> distancesFromCurrToNeighbor = distancesFromCurrToNeighbor(curr);
				for(MapNode neighbor : curr.getNeighbors()) {
					if(!visited.contains(neighbor)) {
						double distanceOfNode = curr.getActualDistance() + distancesFromCurrToNeighbor.get(neighbor);
						if (distanceOfNode < neighbor.getActualDistance()) {
							nodeSearched.accept(neighbor.getLocation()); // hook
	...
	private HashMap<MapNode, Double> distancesFromCurrToNeighbor(MapNode curr) {
		HashMap<MapNode, Double> result = new HashMap<>();
		for(MapEdge e : curr.getEdges()) {
			result.put(e.getEndNode(), e.getLength());
		}
		return result;
	}
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while(!toExplore.isEmpty()) {
			...
			if(!visited.contains(curr)) {
				...
				for(MapNode neighbor : curr.getNeighbors()) {
					if(!visited.contains(neighbor)) {
						GeographicPoint n  = neighbor.getLocation();
						double distanceToGoal = n.distance(goal);
						double distanceOfNode = curr.getActualDistance() + distancesFromCurrToNeighbor.get(neighbor) +distanceToGoal;
						if (distanceOfNode < neighbor.getActualDistance()) {
							nodeSearched.accept(neighbor.getLocation()); // hook
							neighbor.setActualDistance(distanceOfNode);
							neighbor.setDistance(distanceOfNode);
